...
测试数量：42 个，全部通过。当前工作区在 `milestone/2026-05-29-real-mvp50-scale` 分支，包含真实数据 smoke workflow、人工复核备注、指数基准对比、样本内/样本外验证、报告指标扩展、真实成交回填对账、真实成交持仓回放、跟踪误差报告、MVP 主体全链路脚本和 50 只扩容验证改动。
...
- 一个可维护的初始股票池：MVP 已用 50 只股票跑通两年链路；下一步应加入真实成交额阈值校准并扩到 3-5 年区间。
...
- 50 只真实扩容、请求重试和约束缓存复用正在 `milestone/2026-05-29-real-mvp50-scale` 分支开发；真实链路已跑通，尚待提交合并。
...
- 50 只真实股票池批量同步已经跑通；成交额和流动性过滤阈值仍待校准。
- 指数基准对比：已支持基础指标，并已在真实两年区间验证；待补报告展示和更多基准。
- 行业暴露
- 多股票真实股票池批量回测：50 只股票已跑通 MVP 级验证，待扩到更长历史和更高质量股票池。
...
- 数据规模：MVP smoke 已达到 50 只股票和两年区间；上线前建议扩到 3-5 年区间，继续确认耗时、限流和缓存可维护性。
...
- 下载并保存：
  ...
  - MVP 主体链路：已支持 `make real-mvp` 一键跑 20/50 只股票、两年区间真实数据验证。
