...
make PYTHON=.conda/envs/letsquant/bin/python real-mvp-liquid-local MVP_LIMIT=50 MVP_LIQUID_MIN_AVG_AMOUNT=300000
make PYTHON=.conda/envs/letsquant/bin/python real-mvp-liquid-long MVP_LIMIT=50
...
- 已基于 150 候选池校准默认流动性门槛：`MVP_LIQUID_MIN_AVG_AMOUNT=300000`。该值低于第 50 名 `000532.SZ` 的 `avg_amount=321752.25`，高于第 75 名附近的 `225811.46`，可稳定选满 50 只并排除尾部低流动性候选。
- `real-mvp-liquid-local` 已跑通本地阈值复测链路：不重打真实接口，仅基于已缓存候选池重新生成 `mvp50_liquid.csv`、复权、回测、验证和信号。
- 2021-2024 长历史流动性 Top 50 链路已跑通：`MVP_LONG_START=2021-01-01`、`MVP_END=2024-12-31`、`MVP_LONG_SPLIT=2023-12-29`、`MVP_LIMIT=50`，输出到 `results/real_mvp50_liquid_2021_2024`。
- 长历史同步结果：候选日线 150/150、复权因子 150/150、涨跌停交易日 969 个、停复牌交易日 294 个、沪深 300 指数 1 个；`data/limits/*.complete` 与 `data/suspensions/*.complete` 均为 969 个。
- 长历史完整区间回测输出到 `results/real_mvp50_liquid_2021_2024/backtest`：最终权益 76,696.66，总收益 -23.30%，年化 -6.43%，最大回撤 -50.44%，交易 239 笔，拒单 18 笔；同期沪深 300 总收益 -25.30%，策略超额 2.00%。
- 长历史年度收益：2021 年 22.30%，2022 年 -35.42%，2023 年 -8.82%，2024 年 4.34%。
- 长历史样本内/样本外验证输出到 `results/real_mvp50_liquid_2021_2024/validation`：样本内 2021-2023 年收益 -26.49%、超额 8.37%；样本外 2024 年收益 10.16%、超额 -6.04%、交易 43 笔。
- 长历史信号输出到 `results/real_mvp50_liquid_2021_2024/signal`；本次最新收盘日没有可执行新信号，但 `current_signals.csv` 和 `manual_orders.csv` 已生成。
...
测试数量：43 个，全部通过。当前代码线包含真实数据 smoke workflow、人工复核备注、指数基准对比、样本内/样本外验证、报告指标扩展、真实成交回填对账、真实成交持仓回放、跟踪误差报告、MVP 主体全链路脚本、50 只扩容验证、流动性 Top 50 股票池和 2021-2024 长历史链路改动。
...
- 一个可维护的初始股票池：MVP 已用 150 候选池筛选出的流动性 Top 50 跑通 2021-2024 四年链路，并已设置默认最低成交额阈值 `300000`。
...
- 本地验证已通过：Python 3.10 conda 环境下 `make test` 运行 43 个测试通过，`make compile` 通过，样例 `backtest` 和 `signal` 通过，`data probe --help` 正常。
...
- 流动性 Top 50 股票池已提交到 `milestone/2026-05-29-liquid-mvp-universe`，并已合并到本地 `main`；150 候选池同步和 Top 50 全链路已跑通。
- 流动性最低成交额阈值已提交到 `milestone/2026-05-29-liquidity-threshold`，并已合并到本地 `main`；默认阈值为 `MVP_LIQUID_MIN_AVG_AMOUNT=300000`。
- 2021-2024 长历史流动性 Top 50 链路已提交到 `milestone/2026-05-29-long-history-mvp`，并已合并、推送到 `main`；四年真实链路已跑通。
...
- 150 候选池同步、流动性 Top 50 真实链路、默认最低成交额阈值和 2021-2024 长历史验证已经跑通；更大候选池和多窗口滚动验证待补。
- 指数基准对比：已支持基础指标，并已在真实两年区间验证；待补报告展示和更多基准。
- 行业暴露
- 多股票真实股票池批量回测：流动性 Top 50 已跑通 2021-2024 四年真实验证，待扩候选池和分层股票池。
- 样本内/样本外切分：已支持单 split date 验证，并已在 2021-2024 真实区间跑通，待补滚动窗口。
- 滚动参数验证
...
- 主体链路：已闭合。真实接口到数据同步、复权、股票池、回测、验证和信号生成已经一键跑通。
- 数据规模：MVP 已达到 150 候选池筛选流动性 Top 50、默认最低成交额阈值 300000 和 2021-2024 四年区间；后续主要是更大候选池、更多基准和滚动窗口，不再阻塞主体 MVP。
- 策略可信度：当前基准策略四年总收益 -23.30%，但相对沪深 300 有 2.00% 超额；MVP 可以接受作为框架验证，上线使用前需要策略迭代、滚动验证和风控报告。
...
- 下载并保存：
  ...
  - MVP 主体链路：已支持 `make real-mvp` 一键跑 20/50 只股票、两年区间真实数据验证；已支持 `make real-mvp-liquid` 从候选池筛选流动性 Top 50 后跑完整链路；已支持 `make real-mvp-liquid-long` 跑 2021-2024 四年长历史链路。
...
- 已补：`real-mvp-liquid-long` 可一键同步 2021-2024 候选池真实缓存，筛选流动性 Top 50，并跑完整区间回测、样本内/样本外验证和最新人工信号。
