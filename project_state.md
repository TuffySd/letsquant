...
测试数量：43 个，全部通过。当前工作区在 `main` 分支，包含真实数据 smoke workflow、人工复核备注、指数基准对比、样本内/样本外验证、报告指标扩展、真实成交回填对账、真实成交持仓回放、跟踪误差报告、MVP 主体全链路脚本、50 只扩容验证和流动性 Top 50 股票池改动。
...
- 一个可维护的初始股票池：MVP 已用 150 候选池筛选出的流动性 Top 50 跑通两年链路，并已设置默认最低成交额阈值 `300000`；下一步应扩到 3-5 年区间。
...
- 150 候选池同步、流动性 Top 50 真实链路和默认最低成交额阈值已经跑通；待扩到更长历史。
...
- 数据规模：MVP smoke 已达到 150 候选池筛选流动性 Top 50、默认最低成交额阈值 300000 和两年区间；上线前建议扩到 3-5 年区间，继续确认耗时、限流和缓存可维护性。
