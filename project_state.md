...
测试数量：38 个，全部通过。当前工作区在 `main` 分支，本地 `main` 比 `origin/main` 领先 6 个提交，包含真实数据 smoke workflow、人工复核备注、指数基准对比、样本内/样本外验证、报告指标扩展、真实成交回填对账和真实成交持仓回放改动。
...
- 滚动参数验证
- 实盘成交回填和跟踪误差分析：已支持计划订单与真实成交 CSV 对账和实际持仓回放，待补跟踪误差指标。
