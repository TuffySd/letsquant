...
测试数量：40 个，全部通过。当前工作区在 `main` 分支，本地 `main` 比 `origin/main` 领先 7 个提交，包含真实数据 smoke workflow、人工复核备注、指数基准对比、样本内/样本外验证、报告指标扩展、真实成交回填对账、真实成交持仓回放和跟踪误差报告改动。
...
- 一个可维护的初始股票池：MVP 已用 20 只股票跑通两年链路；下一步应扩到 50 只并加入真实成交额阈值校准。
...
- 真实数据 smoke workflow 和人工复核备注已本地提交到 `milestone/2026-05-27-real-smoke-workflow`，尚未推送远程。
- 指数基准对比已本地提交到 `milestone/2026-05-28-benchmark-metrics`，尚未推送远程。
- 本地 `main` 已快进合并上述七个提交，尚未推送远程。
- 样本内/样本外验证已本地提交到 `milestone/2026-05-28-walkforward-validation`，尚未推送远程。
- 报告指标扩展已本地提交到 `milestone/2026-05-28-report-metrics`，尚未推送远程。
- 真实成交回填对账已本地提交到 `milestone/2026-05-28-fill-reconciliation`，尚未推送远程。
- 真实成交持仓回放已本地提交到 `milestone/2026-05-28-position-replay`，尚未推送远程。
- 跟踪误差报告已本地提交到 `milestone/2026-05-28-tracking-diff`，尚未推送远程。
- MVP 主体全链路脚本和文档已在 `milestone/2026-05-29-real-mvp-pipeline` 分支开发；`make real-mvp` 已跑通真实接口，当前结果尚待提交合并。
...
- 50 只及以上真实股票池批量同步后的成交额和流动性过滤校准；20 只股票、两年区间已经用真实接口跑通。
- 指数基准对比：已支持基础指标，并已在真实两年区间验证；待补报告展示和更多基准。
- 行业暴露
- 多股票真实股票池批量回测：20 只股票已跑通 MVP 级验证，待扩容到 50 只和更长历史。
...
- 数据规模：MVP smoke 已达到 20 只股票和两年区间；上线前建议扩到 50 只和 3-5 年区间，确认耗时、限流和缓存可维护性。
...
- 下载并保存：
  ...
  - MVP 主体链路：已支持 `make real-mvp` 一键跑 20 只股票、两年区间真实数据验证。
...
- 已补：`real-mvp` 可一键刷新股票基础信息、生成 20 只股票 MVP 股票池、同步两年真实日线/复权/涨跌停/停复牌/指数数据、生成前复权行情、跑完整区间回测、样本内/样本外验证和最新人工信号。
