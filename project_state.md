...
测试数量：20 个，全部通过。当前工作区在 `milestone/2026-05-27-universe-builder` 分支，包含尚未提交的 P0 股票池生成改动。
...
- 当前未提交文件包括：`.gitignore`、`README.md`、`project_state.md`、`src/letsquant/cli.py`、`src/letsquant/data/universe.py`、`tests/test_data_sync.py`、`tests/test_universe.py`。
...
- 下载并保存：
  ...
  - 指数行情：已支持指定指数缓存。
  - 股票池：已支持从 `stock_basic.csv` 生成基础 universe。

验收标准：

- 可以指定股票池和日期范围自动更新数据。
- 回测不再依赖手工 CSV。
- 待补：增加成交额/流动性过滤，并用小型真实股票池跑第一轮批量同步。
