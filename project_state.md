...
测试数量：13 个，全部通过。当前工作区在 `milestone/2026-05-27-p0-market-cache` 分支，包含尚未提交的 P0 扩展数据缓存改动。
...
- 当前未提交文件包括：`.gitignore`、`README.md`、`project_state.md`、`src/letsquant/cli.py`、`src/letsquant/data/tushare_source.py`、`tests/test_data_sync.py`。
...
- 可以指定股票池和日期范围自动更新数据。
- 回测不再依赖手工 CSV。
- 待补：用 `adj_factor` 生成前复权/后复权行情目录，并增加股票池过滤器。
