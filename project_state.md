...
测试数量：21 个，全部通过。当前工作区在 `milestone/2026-05-27-liquidity-filter` 分支，包含尚未提交的流动性股票池过滤改动。
...
- 当前未提交文件包括：`README.md`、`project_state.md`、`src/letsquant/cli.py`、`src/letsquant/data/universe.py`、`tests/test_universe.py`。
...
- 可以指定股票池和日期范围自动更新数据。
- 回测不再依赖手工 CSV。
- 待补：用小型真实股票池跑第一轮批量同步，并补 CLI 回测/信号的 symbols-file 覆盖能力。
