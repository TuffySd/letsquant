...
测试数量：12 个，全部通过。当前工作区在 `milestone/2026-05-27-market-constraints` 分支，包含尚未提交的真实市场约束小步改动。
...
- 当前未提交文件包括：`.gitignore`、`README.md`、`project_state.md`、`src/letsquant/cli.py`、`src/letsquant/data/csv_source.py`、`src/letsquant/execution/backtester.py`、`src/letsquant/models.py`、`src/letsquant/reports.py`、`tests/test_backtester.py`、`tests/test_data_sync.py`。
...
- 前复权价格生成或读取。
- 停牌日跳过成交：回测已支持字段级拒单，待补数据同步合并。
- 涨停不能买入，跌停不能卖出：回测已支持字段级拒单，待补数据同步合并。
- ST 和退市风险过滤。
- 上市未满 N 日过滤。
- 日均成交额过滤。
- 股票池生成器。
