...
real-mvp: real-check-env
	PYTHONPATH=$(SRC_PATH) $(PYTHON) -m letsquant.cli data sync --provider tushare --symbols 000001.SZ --start-date $(MVP_START) --end-date $(MVP_START) --cache-dir data/daily --with-stock-basic --request-interval $(MVP_REQUEST_INTERVAL)
	...
	PYTHONPATH=$(SRC_PATH) $(PYTHON) -m letsquant.cli data sync --provider tushare --symbols-file $(MVP_UNIVERSE) --limit $(MVP_LIMIT) --start-date $(MVP_START) --end-date $(MVP_END) --cache-dir data/daily --with-adj-factor --with-constraints --index-symbols $(MVP_INDEX_SYMBOLS) --request-interval $(MVP_REQUEST_INTERVAL)
